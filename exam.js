...
function isPictorialQuestion(q){const id=String(q?.id||'').trim();const type=String(q?.type||'').toLowerCase();const hasImage=!!String(q?.image||q?.imageUrl||q?.image_url||'').trim();return type==='pictorial'||(/^bo-2\.2-\d{3}$/.test(id)&&hasImage)}$/.test(id)||String(q?.type||'').toLowerCase()==='pictorial'}
