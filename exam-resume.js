/* Resume v6: preserve exact paper before submission, clear it after final submission. */
(function(){
  ...
  const read=()=>{try{for(const k of ALL){const v=localStorage.getItem(k);if(v){const x=JSON.parse(v);if(x&&x.examId&&Array.isArray(x.paper)&&x.paper.length)return x}}}catch(e){}return null};
  ...
  function saveCurrent(){if(!state||restoring||resultVisible())return;const i=currentIndex();if(i==null)return;state.questionIndex=i;state.answers=state.answers||{};const c=$('#questionCard');const checked=[...(c?.querySelectorAll('input[type="radio"]:checked,input[type="checkbox"]:checked')||[])];if(checked.length){state.answers[i]=checked.map(e=>({value:e.value||e.dataset.value||e.id||'',index:[...c.querySelectorAll('input[type="radio"],input[type="checkbox"]')].indexOf(e)}));}else if(state.answers[i])delete state.answers[i];write()}
  function saveCandidate(){if(!state||resultVisible())return;for(const [id,key] of [['candidateName','name'],['candidateEmail','email'],['candidateWhatsapp','whatsapp']]){const e=document.getElementById(id);if(e&&e.value)state[key]=e.value}write()}
  function patchFetch(){if(window.__nbResumeV6Fetch)return;window.__nbResumeV6Fetch=true;const original=window.fetch.bind(window);window.fetch=async function(input,init){const url=typeof input==='string'?input:(input?.url||'');const cfg=/exam-api\?action=config(?:&|$)/.test(url);const m=url.match(/[?&]exam=([^&]+)/);const id=m?decodeURIComponent(m[1]):null;
  ...
  function resume(){if(!state?.examId||resuming)return;resuming=true;const target=state.questionIndex;const started=Date.now();const tick=()=>{if(resultVisible()){clearSaved();return}if(Date.now()-started>20000){resuming=false;restoring=false;restoreClick=false;return}
  ...
  document.addEventListener('visibilitychange',()=>{if(resultVisible()){clearSaved();return}if(document.visibilityState==='hidden'){saveCandidate();saveCurrent()}else if(state)setTimeout(resume,200)});
  window.addEventListener('pagehide',()=>{if(resultVisible())clearSaved();else{saveCandidate();saveCurrent()}});
  window.addEventListener('beforeunload',()=>{if(resultVisible())clearSaved();else{saveCandidate();saveCurrent()}});
  const obs=new MutationObserver(()=>{if(resultVisible()){clearSaved();return}if(state&&!resuming&&!restoring)setTimeout(()=>{if(resultVisible()){clearSaved();return}if(active())saveCurrent();else saveCandidate()},80)});
  obs.observe(document.body,{childList:true,subtree:true});
  [300,1000,2000,4000,7000].forEach(ms=>setTimeout(()=>{if(resultVisible()){clearSaved();return}state=read()||state;if(state?.examId)resume()},ms));
})();