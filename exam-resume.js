/* Exact exam resume v7: preserve the exact paper, position, candidate data and every selected answer. */
(function(){
  const KEY='nb_loksewa_exact_resume_v7';
  const OLD=['nb_loksewa_exact_resume_v6','nb_loksewa_exact_resume_v5','nb_loksewa_exact_resume_v4','nb_loksewa_exam_resume_v3','nb_loksewa_exam_resume_v2'];
  const ALL=[KEY,...OLD];
  ...
  function write(){try{if(state)localStorage.setItem(KEY,JSON.stringify({...state,savedAt:Date.now(),version:7}))}catch(e){}}
  ...
  function captureCurrent(){
    ...
    if(checked.length)state.answers[i]=checked;else delete state.answers[i];
    write();
  }
  ...
  function patchFetch(){
    if(window.__nbResumeV7Fetch)return;window.__nbResumeV7Fetch=true;
  ...
  function chooseSaved(){const id=state?.examId;if(!id)return false;const b=[...document.querySelectorAll('#examList .exam-card')].find(x=>x.dataset.id===id);if(!b||b.classList.contains('disabled'))return false;restoreClick=true;b.click();return true}
  ...
  function resume(){
    if(!state?.examId||resuming)return;resuming=true;const target=state.questionIndex;const started=Date.now();
  ...
  document.addEventListener('visibilitychange',()=>{if(resultVisible()){clearSaved();return}if(document.visibilityState==='hidden'){captureCandidate();captureCurrent()}else if(state)setTimeout(resume,200)});
  ...
  const obs=new MutationObserver(()=>{if(resultVisible()){clearSaved();return}if(state&&!resuming&&!restoring)setTimeout(()=>{if(resultVisible()){clearSaved();return}if(active())captureCurrent();else captureCandidate()},80)});
  obs.observe(document.body,{childList:true,subtree:true});
  [300,900,1800,3500,6000].forEach(ms=>setTimeout(()=>{if(resultVisible()){clearSaved();return}state=read()||state;if(state?.examId)resume()},ms));
})();