/* Exact exam resume v11
   ...
   - Restores candidate details, question index, answers and countdown timer.
...
(function(){
  ...
  function timerSecondsFromState(){
    if(!state)return null;
    let base=Number(state.timerSeconds);
    if(!Number.isFinite(base)||base<0){
      const m=clean(state.timerText||'').match(/^(\d+):([0-5]\d)$/);
      if(!m)return null;
      base=Number(m[1])*60+Number(m[2]);
    }
    const savedAt=Number(state.savedAt);
    if(Number.isFinite(savedAt)&&savedAt>0){base-=Math.max(0,(Date.now()-savedAt)/1000)}
    return Math.max(0,Math.floor(base));
  }
  function applySavedTimer(){
    const left=timerSecondsFromState();
    if(left==null)return;
    try{
      if(typeof seconds!=='undefined')seconds=left;
      if(typeof updateTimer==='function')updateTimer();
    }catch(e){}
  }
  function capture(){if(!state||resuming||restoring||resultVisible())return;const i=index(),c=$('#questionCard');if(i==null||!c)return;state.started=true;state.questionIndex=i;state.answers=state.answers||{};const els=[...c.querySelectorAll('input[type=radio],input[type=checkbox]')],checked=els.filter(e=>e.checked).map(e=>({value:String(e.value||e.dataset.value||e.id||''),index:els.indexOf(e)}));if(checked.length)state.answers[i]=checked;else delete state.answers[i];const timer=$('#timer');if(timer&&!timer.hidden){state.timerText=clean(timer.textContent);const m=state.timerText.match(/^(\d+):([0-5]\d)$/);if(m)state.timerSeconds=Number(m[1])*60+Number(m[2])}write();setSession(true)}
  ...
  function installStimulus(){if(window.__NBStimulusCoreRequested)return;window.__NBStimulusCoreRequested=true;const s=document.createElement('script');s.src='exam-stimulus-core.js?v=1.1.0';s.async=false;document.head.appendChild(s)}
