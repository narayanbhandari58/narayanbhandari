/* Exact exam resume v12.1
   - Keeps one exact paper per active session.
   - Restores candidate details, exact question, selected answer and countdown.
   - Does not reset the countdown merely because candidate fields are saved.
   - Restores answers through the real option click handler so exam.js state is updated too.
*/
(function(){
  ...
  function capture(){if(!state||resuming||restoring||resultVisible())return;const i=index(),c=$('#questionCard');if(i==null||!c)return;state.started=true;state.questionIndex=i;state.answers=state.answers||{};
    ...
    const timer=$('#timer');if(timer&&!timer.hidden){state.timerText=clean(timer.textContent);const m=state.timerText.match(/^(\d+):([0-5]\d)$/);if(m){state.timerSeconds=Number(m[1])*60+Number(m[2]);state.timerEndsAt=Date.now()+state.timerSeconds*1000}}write();setSession(true)}
  ...
  document.addEventListener('click',e=>{const b=e.target.closest?.('#startBtn');if(!b||!state||resuming)return;const n=$('#candidateName')?.value.trim()||'',em=$('#candidateEmail')?.value.trim()||'';if(n&&em){state.started=true;state.questionIndex=0;setSession(true);candidate();write()}},true);
