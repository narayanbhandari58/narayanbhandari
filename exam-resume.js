...
(function(){
  ...
  function index(){try{if(typeof current!=='undefined'&&Number.isInteger(Number(current))&&Array.isArray(examQuestions)&&examQuestions.length)return Number(current)}catch(e){}const h=$('#questionCard h2');const m=h&&clean(h.textContent).match(/^(\d+)\s*[.)]/);if(m)return Number(m[1])-1;const bs=[...document.querySelectorAll('#questionNav button')];const i=bs.findIndex(b=>b.classList.contains('active')||b.getAttribute('aria-current')==='true');return i>=0?i:null}
  ...
  function capture(){if(!state||resuming||restoring||resultVisible())return;const i=index(),c=$('#questionCard');if(i==null)return;state.started=true;state.questionIndex=i;state.answers=state.answers||{};try{if(typeof answers!=='undefined'&&answers&&typeof answers==='object')state.answerMap={...answers}}catch(e){}if(c){const buttons=[...c.querySelectorAll('.option')];const selected=buttons.find(b=>b.classList.contains('selected'));if(selected)state.answers[i]={index:Number(selected.dataset.i),value:String(selected.dataset.i??'')};else delete state.answers[i]}const timer=$('#timer');if(timer&&!timer.hidden){state.timerText=clean(timer.textContent);const m=state.timerText.match(/^(\d+):([0-5]\d)$/);if(m){state.timerSeconds=Number(m[1])*60+Number(m[2]);const globalEnd=Number(window.__NBExamExpiresAt||0);const end=Number(state.timerEndsAt);if(globalEnd>0)state.timerEndsAt=globalEnd;else if(!Number.isFinite(end)||end<=0)state.timerEndsAt=Date.now()+state.timerSeconds*1000}}write();setSession(true)}
  ...
  state=read();if(!(activeSession()&&state?.started)){setSession(false);state=null}window.__NBResumeNow=()=>{state=read()||state;if(state?.started&&activeSession()){armResume();resuming=false;resume();return true}return false};function patchSubmitAnswers(){
