/* Exact exam resume v12
   - Keeps one exact paper per active session.
   - Restores candidate details, question index, selected answers and countdown.
   - Supports the exam UI's button-based answer controls and optional WhatsApp.
*/
(function(){
  ...
  function capture(){if(!state||resuming||restoring||resultVisible())return;const i=index(),c=$('#questionCard');if(i==null||!c)return;state.started=true;state.questionIndex=i;state.answers=state.answers||{};
    const buttons=[...c.querySelectorAll('.option')];const selected=buttons.find(b=>b.classList.contains('selected'));
    if(selected){state.answers[i]={index:Number(selected.dataset.i),value:String(selected.dataset.i??'')}}else delete state.answers[i];
    const timer=$('#timer');if(timer&&!timer.hidden){state.timerText=clean(timer.textContent);const m=state.timerText.match(/^(\d+):([0-5]\d)$/);if(m)state.timerSeconds=Number(m[1])*60+Number(m[2])}write();setSession(true)}
  ...
  function restoreAnswers(){const c=$('#questionCard');if(!c||!state)return;const list=state.answers?.[state.questionIndex];if(!list)return;const idx=Number(list.index);const b=[...c.querySelectorAll('.option')].find(x=>Number(x.dataset.i)===idx);if(b){b.classList.add('selected')}}
  ...
  document.addEventListener('click',e=>{const b=e.target.closest?.('#startBtn');if(!b||!state||resuming)return;const n=$('#candidateName')?.value.trim()||'',em=$('#candidateEmail')?.value.trim()||'',wa=$('#candidateWhatsapp')?.value.trim()||'';if(n&&(em||wa)){state.started=true;state.questionIndex=0;setSession(true);candidate();write()}},true);
