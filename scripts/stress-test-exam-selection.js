...
function selectPaper(exam, input){
  ...
  // Mirror live API stimulus hydration and verify each shared group remains contiguous.
  function hydrate(q) {
    const own = String(q?.passage || q?.data || q?.figure || '').trim();
    if (own) return q;
    const source = input.find(item => item !== q && item.section === q.section && item.unit === q.unit && String(item?.passage || item?.data || item?.figure || '').trim());
  ...
  const hydrated = selected.map(hydrate);
  const ordered = [...hydrated];
  const groups = new Map(), singles=[];
  for (const q of ordered) { const raw=String(q?.passage||q?.data||q?.figure||'').trim(); const key=q?.groupId ? 'group:'+q.groupId : raw ? 'stimulus:'+raw : ''; if(!key){singles.push(q);continue;} if(!groups.has(key))groups.set(key,[]);groups.get(key).push(q); }
  const expected = [...groups.values()].flatMap(g=>g).concat(singles);
  const positions=new Map(); expected.forEach((q,i)=>{const raw=String(q?.passage||q?.data||q?.figure||'').trim();const key=q?.groupId?'group:'+q.groupId:raw?'stimulus:'+raw:'';if(key){if(!positions.has(key))positions.set(key,[]);positions.get(key).push(i);}});
  for (const pos of positions.values()) for(let i=1;i<pos.length;i++) if(pos[i]!==pos[i-1]+1) return null;
  return expected;
}
