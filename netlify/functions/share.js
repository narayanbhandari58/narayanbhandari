...
${published ? `<meta property="article:published_time" content="${esc(published)}">` : ''}${modified ? `<meta property="article:modified_time" content="${esc(modified)}">` : ''}${category ? `<meta property="article:section" content="${esc(category)}">` : ''}${tags.map(tag => `<meta property="article:tag" content="${esc(tag)}">`).join('')}
...
<script>(function(){var target=${JSON.stringify(canonical)};var ua=navigator.userAgent||'';var crawler=/facebookexternalhit|Facebot|WhatsApp|Twitterbot|LinkedInBot|Googlebot|bingbot|Slackbot|TelegramBot|Discordbot|Pinterest|Skype/i.test(ua);if(!crawler)window.location.replace(target);})();</script>
</body></html>`;
