...
function signRecovery(username) {
  const exp = Math.floor(Date.now() / 1000) + 600;
  const payload = Buffer.from(JSON.stringify({ sub: username, exp })).toString("base64url");
  const sig = crypto.createHmac("sha256", RECOVERY).update(payload).digest("base64url");
  return `${payload}.${sig}`;
}
function verifyRecoveryToken(token, username) {
  try {
...
exports.handler = async event => {
  ...
  try {
    ...
    if (mode === "verify") {
      return json(200, {
        verified: true,
        recoveryToken: signRecovery(username),
        message: "Recovery Key verified भयो। अब नयाँ password राख्नुहोस्।"
      });
    }
