...
function questionImage(q) { const raw = q?.image || q?.imageUrl || q?.image_url || ''; if (raw) return raw; if (q?.type === 'pictorial' && /^bo-2\.2-\d{3}$/.test(String(q.id || ''))) return `https://raw.githubusercontent.com/narayanbhandari58/narayanbhandari/main/image/exam/branch-officer-2.2/${q.id}.png`; return ''; }
function publicQuestion(q) { return { id: q.id, examIds: q.examIds, section: q.section, unit: q.unit, subject: q.subject, topic: q.topic, category: q.category, level: q.level, type: q.type, format: q.format, q: q.q || q.question, options: q.options, image: questionImage(q), imageAlt: q.imageAlt || q.image_alt || q.topic || 'प्रश्नचित्र', passage: q.passage || '', figure: q.figure || '', data: q.data || '', groupId: groupIdOf(q) } }
