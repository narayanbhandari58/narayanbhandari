...
function questionImage(q) { const raw = q?.image || q?.imageUrl || q?.image_url || ''; if (raw) return raw; if (q?.type === 'pictorial' && /^bo-2\.2-\d{3}$/.test(String(q.id || ''))) return `/image/exam/branch-officer-2.2/${q.id}.png`; return ''; }
